...
Например, неизвестный маршрут возвращает `404` с полями `type`, `title` и `status`. Validation Problem Details сохраняет дополнительный словарь ошибок по полям запроса. Подробный контракт и границы применения описаны в `docs/api/problem-details.md`.
