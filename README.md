...
Перед запуском полного набора тестов запустите Docker Desktop в режиме Linux-контейнеров. Unit-тесты модуля Organizations не требуют Docker.
...
`PostgreSqlFixtureTests` использует Testcontainers для запуска отдельной PostgreSQL 18 с базой `supportflow_tests`, проверяет соединение и применение настоящей миграции модуля Organizations. Контейнер освобождается через `IAsyncLifetime` после тестов класса. При первом запуске может потребоваться загрузка Docker-образов. Локальная база из Docker Compose и её данные для этих тестов не используются.

`CreateOrganizationEndpointTests` запускает API через `WebApplicationFactory`, подменяет строку подключения на контейнерную PostgreSQL и проверяет полный HTTP-сценарий. Успешный тест подтверждает `201 Created`, тело ответа, `Location`, нормализацию имени и сохранение записи. Негативный тест подтверждает Problem Details с кодом `400` и отсутствие новой записи.
...
- добавлена fixture PostgreSQL на Testcontainers и интеграционная проверка подключения к тестовой базе;
...
Код первого vertical slice реализован и покрыт unit- и интеграционными тестами. Этап будет завершён после финальной проверки и слияния pull request.
